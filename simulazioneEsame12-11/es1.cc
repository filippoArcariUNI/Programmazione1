...
list * estrai(list *dipendenti,list * entrate, list * uscite){
  ...
  {
    list * firstEntrate=entrate;
    list * firstUscite=uscite;
    int entrateCounter=0;
    int usciteCounter=0;
    while (firstEntrate!=nullptr)
    {
      
      if (firstEntrate->code==dipendenti->code)
      {
       
        entrateCounter++;
      }
      firstEntrate=firstEntrate->next;
      
    }
    while (firstUscite!=nullptr)
    {
      if (firstUscite->code==dipendenti->code)
      {
    ...
    if (entrateCounter==usciteCounter)
    {
      ...
      // creo la Lista
      list * estratto=new list;
      estratto->code=dipendenti->code;
      estratto->next=nullptr;
      if (rootEstratti==nullptr)
      {
        rootEstratti=estratto;
